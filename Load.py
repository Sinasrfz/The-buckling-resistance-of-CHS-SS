...
D = st.sidebar.number_input("Diameter (D) [mm]", min_value=float(D_min), max_value=float(D_max), step=1.0, value=50.0)
if D < D_min or D > D_max:
    st.sidebar.error(f"D must be between {D_min} and {D_max} mm.")
    error = True

L = st.sidebar.number_input("Length (L) [mm]", min_value=float(L_min), max_value=float(L_max), step=1.0, value=500.0)
if L < L_min or L > L_max:
    st.sidebar.error(f"L must be between {L_min} and {L_max} mm.")
    error = True

t = st.sidebar.number_input("Thickness (t) [mm]", min_value=float(t_min), max_value=float(t_max), step=0.1, value=5.0)
if t < t_min or t > t_max:
    st.sidebar.error(f"t must be between {t_min} and {t_max} mm.")
    error = True

e = st.sidebar.number_input("Edge distance (e) [mm]", min_value=float(e_min), max_value=float(e_max), step=1.0, value=30.0)
if e < e_min or e > e_max:
    st.sidebar.error(f"e must be between {e_min} and {e_max} mm.")
    error = True

f0_2 = st.sidebar.number_input("Proof strength (f0.2) [MPa]", min_value=float(f0_2_min), max_value=float(f0_2_max), step=1.0, value=350.0)
if f0_2 < f0_2_min or f0_2 > f0_2_max:
    st.sidebar.error(f"f0.2 must be between {f0_2_min} and {f0_2_max} MPa.")
    error = True

fu = st.sidebar.number_input("Ultimate strength (fu) [MPa]", min_value=float(fu_min), max_value=float(fu_max), step=1.0, value=750.0)
if fu < fu_min or fu > fu_max:
    st.sidebar.error(f"fu must be between {fu_min} and {fu_max} MPa.")
    error = True

n = st.sidebar.number_input("Strain hardening parameter (n)", min_value=float(n_min), max_value=float(n_max), step=0.1, value=5.5)
