...
model_file = "Combined load CHS SS/ANN_best_model(Load (kN)).joblib"
feature_scaler_file = "Combined load CHS SS/feature_scaler.pkl"
target_scaler_file = "Combined load CHS SS/target_scaler.pkl"
...
st.title("Load Prediction Using ANN")
st.markdown(
    "This app predicts the load capacity (`Load (kN)`) of a structure based on user input parameters "
...
if st.sidebar.button("Predict"):
    if ann_model is None:
        ...
        if scaled_input is not None:
            ...
            st.write(f"**Load Capacity (Load (kN)):** {result:.2f} kN")
